window.RESCENE_SITE_UPDATES = {
  "generatedAt": "2026-08-03T10:57:22.876573+00:00",
  "items": [
    {
      "date": "2026-08-03",
      "title": "chore: critical schedule sync",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "6db8a8b",
      "files": [
        "__pycache__/check-site-links.cpython-313.pyc",
        "data/language-catalog-data.js",
        "data/language-catalog.json",
        "data/pluschat-schedule.json",
        "data/quality-report.json",
        "data/rescene-schedule.ics",
        "data/schedule-data.js",
        "data/schedule.json"
      ]
    },
    {
      "date": "2026-08-03",
      "title": "chore: critical full content sync",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "14f2241",
      "files": [
        "__pycache__/check-site-links.cpython-313.pyc",
        "assets/mv/youtube/QN7V87Hzmr8.jpg",
        "assets/mv/youtube/yAZUO4KqH6s.jpg",
        "assets/optimized/307134592f645da7/yazuo4kqh6s-w480.webp",
        "assets/optimized/f9ed363cf5ac3b42/qn7v87hzmr8-w1280.webp",
        "assets/optimized/f9ed363cf5ac3b42/qn7v87hzmr8-w480.webp",
        "assets/optimized/f9ed363cf5ac3b42/qn7v87hzmr8-w768.webp",
        "data/image-manifest.json"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "fix: repair Daily charts and restore chart navigation",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "012675a",
      "files": [
        "404.html",
        "about.html",
        "analytics.html",
        "article.html",
        "busy-boy-release.html",
        "chants.html",
        "contact.html",
        "data/korean-chart-history/bloom--genie-daily.json"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "chore: update external link report",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "6c0a143",
      "files": [
        "data/external-link-report.json"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "chore: critical schedule sync",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "dce8f9f",
      "files": [
        "__pycache__/check-site-links.cpython-313.pyc",
        "data/language-catalog-data.js",
        "data/language-catalog.json",
        "data/quality-report.json",
        "scripts/__pycache__/add-image-dimensions.cpython-313.pyc",
        "scripts/__pycache__/backfill-korean-chart-history.cpython-313.pyc",
        "scripts/__pycache__/check-external-links.cpython-313.pyc",
        "scripts/__pycache__/check-requested-regressions.cpython-313.pyc"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "chore: critical full content sync",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "41e810e",
      "files": [
        "__pycache__/check-site-links.cpython-313.pyc",
        "data/language-catalog-data.js",
        "data/language-catalog.json",
        "data/mv-candidates-data.js",
        "data/mv-candidates.json",
        "data/quality-report.json",
        "data/seo-status.json",
        "scripts/__pycache__/add-image-dimensions.cpython-313.pyc"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "chore: critical YouTube sync",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "cc06e80",
      "files": [
        "data/youtube-channels.json"
      ]
    },
    {
      "date": "2026-08-02",
      "title": "Add files via upload",
      "description": "GitHubでサイト内容を更新しました。",
      "commit": "880e8d6",
      "files": [
        ".github/workflows/sync-notion-news.yml"
      ]
    },
    {
      "date": "2026-08-01",
      "title": "韓国チャート毎時自動同期を追加",
      "description": "Melon・Genie・Bugs・FLO・VIBE・YouTube Music Korea・Spotify KoreaからRESCENEの対象曲だけを取得し、順位・前回比・最高順位・圏外履歴・推移グラフを保存する機能を追加しました。",
      "files": [
        "korean-charts.html",
        "sync-korean-charts.yml",
        "korean-chart-watchdog.yml"
      ]
    },
    {
      "date": "2026-07-22",
      "title": "サイト機能を一括強化",
      "description": "PWA、お気に入り、共有、関連記事、SNS投稿文、MV承認、公開前プレビュー、アクセス解析、更新履歴を追加しました。",
      "files": [
        "PWA",
        "FAVORITES",
        "SHARE",
        "ANALYTICS"
      ]
    },
    {
      "date": "2026-07-21",
      "title": "ストリーミングとスケジュールリンクを改善",
      "description": "サービス別の詳細手順と、Notionスケジュールの関連リンク・ICS連携を追加しました。",
      "files": [
        "streaming.html",
        "schedule.html"
      ]
    },
    {
      "date": "2026-07-20",
      "title": "OGP・SEOを自動最適化",
      "description": "静的ニュース記事、記事別OGP、構造化データ、サイトマップを自動生成するようにしました。",
      "files": [
        "articles/",
        "sitemap.xml",
        "assets/ogp/"
      ]
    },
    {
      "date": "2026-07-19",
      "title": "統合同期と同期状況ページを追加",
      "description": "Notion・YouTube・MV・SEO・品質検査をまとめて実行できる統合同期を整備しました。",
      "files": [
        "sync-status.html",
        "sync-all-content.yml"
      ]
    },
    {
      "date": "2026-07-19",
      "title": "画像最適化と外部リンク確認を追加",
      "description": "画面幅別WebPと定期的な外部リンク切れチェックを追加しました。",
      "files": [
        "assets/optimized/",
        "external-links.html"
      ]
    }
  ]
};
