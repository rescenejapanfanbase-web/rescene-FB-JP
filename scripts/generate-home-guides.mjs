...
const definitions = [
  ...
  { key: 'streaming', title: 'ストリーミングガイド', summary: 'YouTube、Spotify、Apple Musicなどの応援方法をまとめています。', url: 'streaming.html', category: 'STREAMING', path: 'data/streaming-guide.json' },
...
function gitDirty(file) {
  try {
    return Boolean(execFileSync('git', ['status', '--porcelain', '--', file], { cwd: root, encoding: 'utf8' }).trim());
  } catch {
    return false;
  }
}
...
const guides = definitions.map((definition) => {
  const dirty = gitDirty(definition.path);
  const updatedAt = dirty ? now : (gitValue('%cI', definition.path) || previous[definition.key]?.updatedAt || now);
