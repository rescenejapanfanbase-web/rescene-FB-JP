(()=>{
 ...
 const sortedReleases=releases=>[...releases].sort((a,b)=>{
  const delta=dateValue(a.releaseDate)-dateValue(b.releaseDate);
 ...
 const categorySection=category=>{
  const releases=sortedReleases((payload.releases||[]).filter(item=>item.category===category.key));
 ...
 const renderFilters=()=>{
  ...
  const categoryButtons=[{key:'all',title:'すべて'},...categories.map(category=>({key:category.key,title:category.title}))].map(item=>`<button aria-pressed="${item.key===active}" class="filter${item.key===active?' active':''}" data-disc-filter="${escapeHtml(item.key)}" type="button">${escapeHtml(item.title)}</button>`).join('');
  const sortButtons=`<span class="discography-sort-separator" aria-hidden="true"></span><span class="discography-sort-label">並び順</span><button aria-pressed="${sortOrder==='desc'}" class="filter${sortOrder==='desc'?' active':''}" data-disc-sort="desc" type="button">新しい順</button><button aria-pressed="${sortOrder==='asc'}" class="filter${sortOrder==='asc'?' active':''}" data-disc-sort="asc" type="button">古い順</button>`;
  filterHost.innerHTML=categoryButtons+sortButtons;
  ...
  filterHost.querySelectorAll('[data-disc-sort]').forEach(button=>button.addEventListener('click',()=>{sortOrder=button.dataset.discSort==='asc'?'asc':'desc';render();}));
 };
 ...
 const render=()=>{
  ...
  statusHost.textContent=`公開作品 ${payload.releases.length}件・${sortOrder==='desc'?'新しい順':'古い順'}・最終同期 ${payload.generatedAt?new Intl.DateTimeFormat('ja-JP',{timeZone:'Asia/Tokyo',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(payload.generatedAt)):'未取得'}`;
